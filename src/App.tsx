  ...
  const appendToDf = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    schema.forEach(field => {
      if (field.autoGenerate) {
        switch (field.name) {
          case 'srNo':
            const currentSheetData = df[activeSheet] || [];
            autoGeneratedData[field.name] = String(currentSheetData.length + 1);
            break;
          case 'timestamp':
    ...
    if (role === 'student' && user && user.type === 'student') {
      const logEntry: DataFrameRow = {
        id: Math.random().toString(36).substring(7),
        data: {
          srNo: String((df["Student Submission Logs"] || []).length + 1),
                  ...
                  {schema.map(field => (
                      <div key={field.name} className={field.label.toLowerCase().includes('link') ? "md:col-span-2" : "md:col-span-1"}>
                        <label className="block text-sm sm:text-base font-black text-slate-800 uppercase tracking-[0.08em] mb-2 ml-0.5">
                          {field.label}
                          {field.autoGenerate && <span className="text-mint-600 font-black"> (Auto)</span>}
                        </label>
                        <input
                          name={field.name}
                          type={field.type}
                          required={!field.autoGenerate}
                          disabled={field.autoGenerate}
                          defaultValue={field.autoGenerate ? 'Auto-filled' : ''}
                          className={`w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 sm:px-4 py-2.5 sm:py-3 text-base focus:bg-white focus:border-mint-500 focus:ring-4 focus:ring-mint-500/10 outline-none transition-all text-slate-900 font-semibold ${
                            field.autoGenerate ? 'bg-slate-100 cursor-not-allowed' : ''
                          }`}
                        />
                      </div>
                    ))}
