...
export function Sidebar({ active, onSelect }: SidebarProps) {
  ...
  return (
    <aside className="w-full max-w-80 bg-white border-r border-mint-200 flex flex-col shadow-2xl z-20">
      <div className="p-5 sm:p-8 border-b border-mint-100 bg-white">
      ...
      <nav className="flex-1 overflow-y-auto px-3 sm:px-4 pb-8 sm:pb-10 space-y-1 custom-scrollbar">
        {filtered.map((name: string, index: number) => (
          <button
            ...
            className={`w-full flex items-center justify-between px-4 sm:px-6 py-3.5 sm:py-4 rounded-2xl text-[11px] transition-all group ${
              active === name 
